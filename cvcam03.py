	...
	def __init__(self):
		...
		try:
			...
			if b'NA' in stdout_value:
				self.RestartModem()
				print("pppip.sh -> NA. NO PPP Network Detected. Try Again?...")
			else:
				os.system('sudo pon rnet')
				sleep(5)
				pppip = str(stdout_value)
				print("Found PPP Network " + pppip[2:-3])
				cmds = 'sudo route add default gw %s' % pppip[2:-3]
		...
		while(True):
			...
			try:
				...
				if stDev > self.sdThresh:
					...
					self.filename = '/home/pi/pycam/cam2/x00000003_%s.jpg' % timestampMessage
					fname = 'x00000003_%s.jpg' % timestampMessage
